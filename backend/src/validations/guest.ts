...
const transportationModes = ['private_car', 'bus', 'van', 'motorcycle', 'plane', 'other'] as const;
const purposes = ['leisure', 'business', 'event', 'others'] as const;
...
const guestDetail = z.object({
  ...
  count: z.coerce.number().int().min(1).max(1000, 'Count must be between 1 and 1000'),
});
...
export const guestRecordSchema = guestRecordBase
  ...
  .refine(
    (data) => {
      ...
      if (data.guests && data.guests.length > 0) return true;
      return (
        typeof data.nationality === 'string' && data.nationality.length > 1 &&
    ...
    {
      message:
        'Must provide guest demographics (either supply guests array or nationality/gender/age)',
      path: ['guests'],
    }
  );
...
export type GuestRecordBody = z.infer<typeof guestRecordSchema>;
