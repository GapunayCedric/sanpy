...
import type { LoginBody, RegisterBusinessBody, ForgotPasswordBody, ResetPasswordBody } from '../validations/auth.js';
...
export async function login(req: Request, res: Response): Promise<void> {
  ...
  const users = await query<{ id: number; email: string; password_hash: string; role: string; status: string }[]>(
  ...
  const token = signToken({
    userId: user.id,
    email: user.email,
    role: user.role as 'business' | 'admin',
  });
...
export async function registerBusiness(req: Request, res: Response): Promise<void> {
  const body = req.body as RegisterBusinessBody;
  const files = req.files as { permitFile?: Express.Multer.File[]; validIdFile?: Express.Multer.File[] } | undefined;
  const permitFile = files?.permitFile?.[0];
  const validIdFile = files?.validIdFile?.[0];

  const existing = await query<{ id: number }[]>('SELECT id FROM users WHERE email = ?', [body.email]);
  ...
  const userResult = await query<{ insertId: number }>(
    ...
    [body.email, passwordHash, 'business', 'pending']
  );
  ...
  await query(
    `INSERT INTO businesses (user_id, business_name, permit_number, owner_name, address, barangay, contact_number, permit_file_url, valid_id_url)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [
      userId,
      body.businessName,
...
export async function resetPassword(req: Request, res: Response): Promise<void> {
  ...
  const tokens = await query<
    { id: number; user_id: number; expires_at: Date; used: number }[]
  >(
...
export async function me(req: Request, res: Response): Promise<void> {
  ...
  >('SELECT id, email, role, status FROM users WHERE id = ?', [req.user.userId]);
  const user = users[0];
  if (!user) {
  ...
  if (user.role === 'business') {
    const businesses = await query<
      { id: number; business_name: string; permit_number: string; address: string; contact_number: string; owner_name: string }[]
    >('SELECT id, business_name, permit_number, address, contact_number, owner_name FROM businesses WHERE user_id = ?', [user.id]);
